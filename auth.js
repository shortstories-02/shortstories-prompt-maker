...
(function(){
  ...
  async function rejectAuthenticatedSession(c, reason){
    let message;
    ...
    // The SIGNED_OUT event generated by this signOut must not rebuild
    // the empty login screen over the rejection message.
    suppressNextSignedOut = true;

    await c.auth.signOut({scope:'local'}).catch((error) => {
      console.warn('Rejected session signOut:', error);
    });
    ...
    // Safety reset in case Supabase did not emit SIGNED_OUT.
    window.setTimeout(() => {
  ...
  async function showApp(user){
    ...
    // IMPORTANT: NEVER reveal the app until the profile/license check
    // has completed successfully. This is also used on refresh/session
    // restoration.
    const access = await getProfileAccess(c, user);

    if(!access.allowed){
      await rejectAuthenticatedSession(c, access.reason);
      return;
    }
  ...
  async function submit(e){
    ...
    // This flag is set BEFORE signInWithPassword so the SIGNED_IN
    // callback can never open the app on its own.
    ...
    try{
      ...
      if(!data?.user) throw new Error('Login berhasil tetapi data pengguna tidak ditemukan.');

      // Explicit login owns the entire access decision. The auth event
      // listener is deliberately blocked while loginInProgress is true.
      const access = await getProfileAccess(c, data.user);
      ...
      if(!suppressNextSignedOut && !pendingAuthMessage){
